...
import { buildComponent, getValuesGroup } from './utils/const'
...
export const initAtom = atom(undefined, (getter, setter, { dataConfig }: PivotProps) => {
  ...
  setter(
    ...
    columns.map((prop, index) => {
      ...
      // 格式某些cell
      meta.forEach((meItem) => {
        if (meItem?.formatter && groupedValues?.[meItem.field]?.includes(prop)) {
          columnOption.renderComponent = buildComponent(meItem?.formatter)
        }
      })
